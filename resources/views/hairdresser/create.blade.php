...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __(' Create New Hairdresser') }}
        </h2>
    </x-slot>
    <div class="container">
        <div class="header">
            <h1>JJ Hair Salon-New Hairdresser</h1>
        </div>
        
        <form action="{{route('hairdresser.store')}}" method="POST" enctype="multipart/form-data">
        @csrf
            ...
            <div class="form-group">
                <label for="hairdresser">Position</label>
                    <input type="text" name="position" id="position" class="form-control" style="width:100%">
                            @error('position')
                            <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                            @enderror
            </div>
            ...
            <div class="form-group">
                <label for="hairdresser">Job description</label>
                    <textarea type="text" name="job_description" id="job_description" class="form-control" style="width:100%"></textarea>
